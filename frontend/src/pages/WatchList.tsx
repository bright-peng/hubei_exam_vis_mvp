...
const WatchList: React.FC = () => {
    ...
    const totalStats = positions.reduce((acc, pos) => ({
        quota: acc.quota + ((getRecordValue(pos, '招录人数') as number) || 0),
        applicants: acc.applicants + ((getRecordValue(pos, '报名人数') as number) || 0),
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: '职位名称',
            ellipsis: true,
            render: (val: string) => <ArcoTooltip content={val}>{val}</ArcoTooltip>
        },
        {
            title: '用人单位',
            dataIndex: '用人单位',
        ...
        },
        {
            ...
            sorter: (a: Position, b: Position) =>
                ((getRecordValue(a, '报名人数') as number) || 0) - ((getRecordValue(b, '报名人数') as number) || 0),
            render: (val: number) => <Text bold color="arcoblue">{val?.toLocaleString()}</Text>
        },
        {
            title: '竞争比',
            width: 100,
            align: 'center' as const,
            sorter: (a: Position, b: Position) =>
                ((getRecordValue(a, '竞争比') as number) || 0) - ((getRecordValue(b, '竞争比') as number) || 0),
            render: (_: unknown, record: Position) => {
                const ratio = (getRecordValue(record, '竞争比') as number) || 0
                const isHot = ratio > 50
                return <Badge status={isHot ? 'error' : 'success'} text={`${ratio}:1`} />
            }
        },
        {
            title: '操作',
            ...
            render: (_: unknown, record: Position) => (
                <Space>
                    ...
                    <Button type="text" status="danger" size="mini" icon={<IconDelete />} onClick={() => handleRemoveCode(getRecordValue(record, '职位代码') as string)} />
                </Space>
            )
        }
    ]
            ...
            <Row gutter={[24, 24]} style={{ marginTop: 24 }}>
                ...
                <Col xs={24} md={8}>
                    ...
                    >
                        ...
                        <Text type="secondary">已关注 ({watchCodes.length}):</Text>
                                ...
                                <Alert
                                    ...
                                    content={
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                                            {notFound.map(code => (
                                                <Tag key={code} closable onClose={() => handleRemoveCode(code)} size="small" color="red">
                                                    {code}
                                                </Tag>
                                            ))}
                                        </div>
